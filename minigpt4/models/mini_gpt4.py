...
from models.llama2 import ModifiedLlama2
...
class MiniGPT4(Blip2Base):
    ...
    def __init__(
        ...
        if self.low_resource:
            self.llama_model = ModifiedLlama2.from_pretrained(
                llama_model,
    ...
    def forward(self, samples):
        ...
        if isinstance(instruction, str):
            instruction_list = [instruction] * len(img_embeds)
            instruction = [ins + qst for ins, qst in zip(instruction_list, samples["text"])]

        img_embeds, atts_img = self.prompt_wrap(img_embeds, atts_img, instruction)

        self.llama_tokenizer.padding_side = "right"
        text = [t + self.end_sym for t in samples["answer"]]  # QST: should it contain instruction?
